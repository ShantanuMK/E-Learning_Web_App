<?php
  ...
  if (isset($_POST['studymaterial'])) {
    header("Location:studym.php");
  }
  if (isset($_POST['assignment'])) {
    header("Location:stuassign.php");
  }
  if (isset($_POST['exam'])) {
    ...
    $eetime=date("H-i",strtotime($r12['eetime']));
    $ssid=$_SESSION['sid'];
    $eeid=$r12['eid'];
    if ((strtotime($current_date)==strtotime($r12['edate'])) and (strtotime($current_time)>=strtotime($extime)) and (strtotime($current_time)<strtotime($eetime))) {
        $sql="SELECT * from examgiven where eid=$eeid and sid=$ssid";
        $gets=mysqli_query($conn,$sql);
        if (mysqli_num_rows($gets)==0) {
          $_SESSION['eid']=$r12['eid'];
      ...
      echo '<script>alert("Exam is Not Yet Started")</script>';
    }
  }
?>
<!DOCTYPE html>
